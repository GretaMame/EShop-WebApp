  ...
  <div class="orders-table">
    ...
    <el-container v-loading="loading">
      ...
      <el-main class="table-div">
        <el-table class="table" :data="items" :stripe="true" size="medium" :fit="true" :header-cell-style="() => { return {'text-align': 'center'} }">
          <el-table-column label="Order no." prop="orderNumber">
            ...
            <template slot-scope="scope">
              <el-dropdown>
                ...
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item v-for="(item, index) in buildPossibleStatus(scope.row)" :key="`item-${index}`" @click.native="changeStatus(scope.row,item)">
                    {{ item }}
                  </el-dropdown-item>
                </el-dropdown-menu>
              </el-dropdown>
  ...
  export default {
    ...
    methods: {
      ...
      fetchData () {
        ...
        var rowsPromise = this.axios.get(
          `odata/AdminOrders?$count=true&$top=0${filterText ? `&$filter=${filterText}` : ''}`)
