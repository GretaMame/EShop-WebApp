<template>
  <div class="users-table">
    <el-dialog :title="'Orders of ' + selectedUser.email" v-if="showUserDetails" :visible.sync="showUserDetails" width="70%">
      <el-container v-loading="userDetailsLoading" class="user-details-window">
        <el-main>
          <el-card class="gd_order" v-for="order in selectedUserOrders" :key="order.id" shadow="hover">
    ...
    <el-container v-loading="loading">
            ...
            <template slot-scope="scope">
              <el-dropdown>
                ...
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item v-for="(item, index) in buildPossibleRoles(scope.row)" :key="`item-${index}`" @click.native="changeRole(scope.row,item)">
                    {{ item }}
                  </el-dropdown-item>
                </el-dropdown-menu>
              </el-dropdown>
      ...
      <el-footer>
        <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :page-sizes="pageOptions" :page-size="perPage"
    ...
    methods: {
      ...
      fetchData () {
        ...
        var itemsPromise = this.axios.get(
          `odata/Users?$skip=${this.perPage * (this.currentPage - 1)}&$top=${this.perPage}${filterText ? `&$filter=${filterText}` : ''}`
        )
